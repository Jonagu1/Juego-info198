...
void MainWindow::onSocketReadyRead()
{
    ...
    for (const QString& message : messages)
    {
        ...
        // ---
            ...
            // Mensaje nuevo: "TURN:Ana:Beto"
            QString currentPlayer = message.section(':', 1, 1);
            QString nextPlayer = message.section(':', 2, 2); // El 3er elemento

            if (currentPlayer == m_playerName) {
                // --- ¡LUZ VERDE! ---
                ui->nextTurnButton->setEnabled(true);
                ui->turnStatusLabel->setText("¡ES TU TURNO!");
                ui->turnStatusLabel->setStyleSheet(
                    "background-color: #4CAF50; color: white; padding: 5px; font-weight: bold; border-radius: 5px;"
                    );
                ...
                ui->turnStatusLabel->setStyleSheet(
                    "background-color: #FFC107; color: black; padding: 5px; font-weight: bold; border-radius: 5px;"
                    );
            ...
            // Desactivamos el botón (ya no necesitamos estilos aquí)
            ui->nextTurnButton->setEnabled(false);
